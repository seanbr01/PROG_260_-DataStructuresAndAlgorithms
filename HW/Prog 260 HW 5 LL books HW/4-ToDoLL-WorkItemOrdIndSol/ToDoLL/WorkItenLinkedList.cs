...
namespace ToDoLL
{
    class WorkItenLinkedList
    {
        ...
        protected class LinkedListNode
        {
            ...
            public Book nodeBook; // holds an object with real data
            ...
            public LinkedListNode(Book pBook)
            {
                nodeBook = pBook; // set the value of the data object to the one passed in
                //pWorkItem.Value;
                //pWorkItem.What;
                //pWorkItem.Why;
        ...
        public void InsertAtFront(Book pBook)
        {
            LinkedListNode newNode = new LinkedListNode(pBook);  // create a new node
        ...
        public Book RemoveFromFront()  // just had to change method from int to return a WorkItem instead
        {
            ...
            else
            {
                ...
                returnBook = frontOfList.nodeBook; // get the object from the node at the front of the list
        ...
        public void InsertInOrder(Book pBook)
        {
            ...
            // this has to changd
            if (frontOfList == null || frontOfList.nodeBook.Rating >= pBook.Rating)  // watch the pointers to see what happens
            {
                InsertAtFront(pBook);
            }
            else  // we have to traverse the LL and find where to add a new node
            {
                ...
                // this line changed
                while (current.node_next_pointer != null && current.node_next_pointer.nodeBook.Rating < pBook.Rating)
                {
        ...
        public void  InsertAt(Book pBook, int index)
        {
            // if list is empty, or if the desired index is 1, then we want front to point to our new node.
            if (frontOfList == null || index == 1)
            {
                InsertAtFront(pBook);  // so just call our insert at front, which already handles the zero items or exisitng items correctly
                return; // we are done  
            }

            LinkedListNode newNode = new LinkedListNode(pBook); // create our new node to be added, and pass in the object
