...
namespace Birthday
{
    class Bday_BST
    {
        ...
        {
            public Birthday BdayObject;  // uses bstKey as "the data item" in our prior linked lists
            ...
            public BSTnode(Birthday key)  // constuctor to create a new node.  
            {
                BdayObject = key;
            }
            ...
            public void AddRec_BSTnode(Birthday keyParam)  // TRICKY ***** this is a method of the BSTnode class, not the outer BST class!
            {
                ...
                if (keyParam == this.BdayObject)
                {
                    throw new ApplicationException("Msg from BSTnode class: tried to add a duplicate value");
                }

                if (keyParam.Date < this.BdayObject.Date)  // says this new node needs to go to the left of here
                {
            ...
            public Birthday FindRec_BSTnode(int target)  // TRICKY ***** this is a method of the BSTnode class, not the outer BST class!
            {
                ...
                if (target == current.BdayObject.Date) // if the current node has the correct value
                {
                    return current.BdayObject;  // we have a match; other wise, we need to move down the right or left branch
                }
                else if (target > current.BdayObject.Date) // if target is bigger, we go down the right fork
                {
                    if (current.RightNode == null)
                    {
                        throw new ApplicationException("no such date");  // we reached the end of the rigth side without finding a match
                    }
                    else
                    {
                ...
                else // must have been less than, so go down left fork
                {
                    ...
                    {
                        throw new ApplicationException("no such date");  // we reached the end of the left side without finding a match
                    }
                    else
                    {
        ...
        // the recursive method in our internal class to walk the tree
        {
            if (bstTop == null) // deal with an empty BST
            {
                throw new ApplicationException("no such date"); ;   // can't be here if there are none
            }
