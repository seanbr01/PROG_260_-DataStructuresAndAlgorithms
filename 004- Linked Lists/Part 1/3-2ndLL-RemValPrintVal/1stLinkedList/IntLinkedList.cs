...
namespace _1stLinkedList
{
    public class IntLinkedList
    {
        ...
        public void Print()
        {
            ...
            {
                Console.WriteLine(current.node_data);   // 3.	Work (that is done at each iteration)
        ...
        public bool Find(int target)
        {
            ...
            while (current != null)              // 2.	Iteration Logic
            {
                if (current.node_data == target)   // 3.	Work (that is done at each iteration)
                {
                    return true;
                }
                current = current.node_next_pointer;             // 2.	Iteration Logic
        ...
        public bool RemoveByValue(int target) // will return true if found one, false if there was not one
        {
            ...
            else // list was not empty, and the first item was not the one we wanted
            {
                ...
                // we remove, we don't want to move current to the one to delete, but rather, have it one node in back of it
                while (current.node_next_pointer != null)              // this exits while we are at the n-1 node, but looking at the value of the last node
                {
                    if (current.node_next_pointer.node_data == target)   // look ahead one node, is this value what we are looking for?
                    {
        ...
        public bool PrintAllMatching(int target)
        {
            ...
            while (current != null)              // 2.	Iteration Logic
            {
                if (current.node_data == target)   // 3.	Work (that is done at each iteration)
                {
                    Console.WriteLine("Found an entry {0} with the value {1} .", counter, target);
                    myReturn = true;
                }
                current = current.node_next_pointer;             // 2.	Iteration Logic
                counter++;
